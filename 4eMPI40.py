...
#how to run: mpiexec -n 6 python 4eMPI40.py
...
T = np.linspace(2.290-0.042,2.290+0.042,10) #temperature. checking the area around the C-peak.
beta = 1/(k*T)
L = 100 #dimensions
J = 1
...
if rank ==0:
    ...
    C_peak = np.amax(C_avg); print('C-peak = ', C_peak)
    index = np.argmax(C_avg); print('index = ', index)
    Tc = T[index]; print('Tc = ', Tc)
